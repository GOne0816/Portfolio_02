  ...
  return (
    <div className="max-w-3xl mx-auto py-20 px-4">
      ...
      <div className="space-y-4">
        {faqs.map((faq, idx) => (
          <div key={idx} className="bg-[#63e]/10 rounded-xl ring-2 ring-[#63e]/30">
            <button
              className="w-full text-left px-6 py-4 focus:outline-none flex justify-between items-center"
              onClick={() => toggle(idx)}
            >
              <span className="text-lg font-semibold text-[#63e]">{faq.question}</span>
              <span className="text-2xl text-[#63e]">{openIndex === idx ? "−" : "+"}</span>
            </button>
            {openIndex === idx && (
              <div className="px-6 pb-4 text-gray-200 text-base">
                {faq.answer}
              </div>
            )}
          </div>
